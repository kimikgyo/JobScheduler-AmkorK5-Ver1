    ...
    public class MissionFinishedHistoryRepository
    {
        ...
        private void createTable()
        {
            ...
            string checkTableQuery = @"
               IF OBJECT_id('dbo.[JobScheduler_MissionFinishedHistory]', 'U') IS NULL
                BEGIN
                    CREATE TABLE dbo.[JobScheduler_MissionFinishedHistory]
                    (
            ...
            lock (_lock)
            {
                using (var con = new SqlConnection(connectionString))
                {
                    const string INSERT_SQL = @"
                            INSERT INTO [JobScheduler_MissionFinishedHistory]
                                   (
                                       [orderId]
                                      ,[jobId]
                                      ,[guid]
        ...
        public List<Mission> FindHistoryOrderId(string orderId)
        {
            ...
            {
                histories.Clear();
                var sql = @"SELECT * FROM [JobScheduler_MissionFinishedHistory] WHERE orderId = @orderId";
                using (var con = new SqlConnection(connectionString))
                {
        ...
        public List<Mission> FindHistoryJobId(string jobId)
        {
            ...
            {
                histories.Clear();
                var sql = @"SELECT * FROM [JobScheduler_MissionFinishedHistory] WHERE jobId = @jobId";
                using (var con = new SqlConnection(connectionString))
                {
        ...
        public List<Mission> FindHistory(DateTime start, DateTime end)
        {
            ...
            {
                histories.Clear();
                var sql = @"SELECT * FROM [JobScheduler_MissionFinishedHistory] WHERE finishedAt >= @start AND finishedAt <= @end";
                using (var con = new SqlConnection(connectionString))
                {
        ...
        public void PastDataDelete(DateTime endAt)
        {
            lock (_lock)
            {
                using (var con = new SqlConnection(connectionString))
                {
                    var sql = @"DELETE [JobScheduler_MissionFinishedHistory] WHERE finishedAt <= @endAt";
